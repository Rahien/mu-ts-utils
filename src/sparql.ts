...
function newSparqlClient() {
  let options = { requestDefaults: { headers: {} as Record<string, string> } };
...
function executeQuery(queryString: string) {
  return newSparqlClient()
    ...
    .then((response: any) => {
      ...
      function maybeParseJSON(body: any) {
        // Catch invalid JSON
        try {
...
function sparqlEscapeDecimal(value: number | string) {
  return '"' + Number.parseFloat(`${value}`) + '"^^xsd:decimal';
}

function sparqlEscapeInt(value: number | string) {
  return '"' + Number.parseInt(`${value}`) + '"^^xsd:integer';
}

function sparqlEscapeFloat(value: number | string) {
  return '"' + Number.parseFloat(`${value}`) + '"^^xsd:float';
}

function sparqlEscapeDate(value: string | Date) {
  return '"' + new Date(value).toISOString().substring(0, 10) + '"^^xsd:date'; // only keep 'YYYY-MM-DD' portion of the string
}

function sparqlEscapeDateTime(value: string | Date) {
  return '"' + new Date(value).toISOString() + '"^^xsd:dateTime';
}

function sparqlEscapeBool(value: boolean | null | undefined) {
  return value ? '"true"^^xsd:boolean' : '"false"^^xsd:boolean';
}

function sparqlEscape(value: any, type: string) {
  switch (type) {
    case "string":
